...
const googleSansStack = `"Google Sans", system-ui, sans-serif`;
...
export default function NewsOriginV2() {
  const [url, setUrl] = useState(defaultUrl);
  const [metadata, setMetadata] = useState<MetadataV2 | null>(null);
  ...
  useEffect(() => {
    document.title = "ARTICLEOrigin | CORS Proxy";
  ...
  }/api/v2/get?url=${encodeURIComponent(url)}");\nconst result = await response.json();`;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setLoading(true);
      const response = await fetch(getApiUrl(`/api/v2/get?url=${encodeURIComponent(validUrl.toString())}`));
      ...
      <main className="flex-grow flex flex-col items-center gap-6 p-4 pt-10 sm:gap-12 sm:p-12 md:p-24">
        ...
        <div className="w-full max-w-4xl flex justify-between items-center">
            ...
            <div className="flex gap-4 text-xs font-bold uppercase tracking-widest opacity-50">
                ...
                <Link to="/tools/newsorigin/v2" className="text-blue-600 dark:text-blue-400">Article</Link>
        ...
        <div className="relative m-auto flex flex-col items-center after:absolute after:top-0 after:-z-20 after:h-[180px] after:w-[180px] after:animate-pulse after:bg-gradient-to-r after:from-sky-200 after:via-blue-200 after:blur-2xl after:content-[''] after:sm:w-[360px] before:lg:h-[360px] before:dark:bg-gradient-to-br before:dark:from-transparent before:dark:to-blue-700/10 after:dark:from-sky-900 after:dark:via-[#0141ff]/40">
        ...
        <div className="w-full max-w-4xl font-sans">
            ...
            <div className="rounded-xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-md dark:border-neutral-800 dark:bg-neutral-900">
                ...
                GET /api/v2/get?url=example.com/image.png
                <br/>
                &rarr; Redirects to /api/get?url=...
                </div>
            </div>
            </div>
        </div>
