...
import { ClipboardDocumentListIcon, XMarkIcon, ArrowLeftIcon } from "@heroicons/react/20/solid";
...
export default function NewsOriginJSON() {
  const [url, setUrl] = useState(defaultUrl);
  const [fullMetadata, setFullMetadata] = useState<any>(null);
  const [displayedMetadata, setDisplayedMetadata] = useState<any>(null);
  ...
  useEffect(() => {
    document.title = "JSONOrigin | CORS Proxy";
    if (typeof window !== "undefined") {
      setHost(window.location.host);
      setProtocol(window.location.protocol);
      (window as any).showTooltip = (url: string, x: number, y: number) => {
  ...
  }/api/get?url=${encodeURIComponent(url || "https://example.com")}${fieldsParam}");\nconst result = await response.json();`;
      ...
      <main className="flex-grow flex flex-col items-center gap-6 p-4 pt-10 sm:gap-12 sm:p-12 md:p-24">
        ...
        <div className="w-full max-w-4xl flex justify-between items-center">
            ...
            <div className="flex gap-4 text-xs font-bold uppercase tracking-widest opacity-50">
                ...
                <Link to="/tools/newsorigin/json" className="text-blue-600 dark:text-blue-400">JSON</Link>
                <Link to="/tools/newsorigin/hentai" className="hover:text-blue-600">Manga</Link>
            </div>
        </div>

        <div className="relative m-auto flex flex-col items-center after:absolute after:top-0 after:-z-20 after:h-[180px] after:w-[180px] after:animate-pulse after:bg-gradient-to-r after:from-amber-200 after:via-orange-200 after:blur-2xl after:content-[''] after:sm:w-[360px] before:lg:h-[360px] before:dark:bg-gradient-to-br before:dark:from-transparent before:dark:to-orange-700/10 after:dark:from-amber-900 after:dark:via-[#ffae01]/40">
                ...
                <div className="p-6 bg-[#282c34]">
                    <pre className="overflow-x-auto">
                        ...
                        __html: hljs.highlight(JSON.stringify(displayedMetadata, null, 2), { language: "json" }).value
                            ...
                            if (url.match(/\.(?:png|jpg|jpeg|gif|svg|webp|avif|bmp|ico)(?:\?.*)?$/i) || url.includes("/api/get?url=")) {
        ...
        <div className="w-full max-w-4xl font-sans">
            ...
            <div className="rounded-xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-md dark:border-neutral-800 dark:bg-neutral-900 sm:col-span-2 lg:col-span-1">
                ...
                <div className="rounded bg-neutral-100 p-3 text-xs font-mono dark:bg-neutral-800 overflow-x-auto">
                <pre>GET /api/get?url=...&fields=title,links</pre>
                </div>
            </div>
            </div>
        </div>
        ...
        {tooltip.show && (
            ...
            >
            <img
                src={tooltip.url.startsWith("http") && !tooltip.url.includes("/api/get?url=") ? getApiUrl(`/api/get?url=${encodeURIComponent(tooltip.url)}`) : tooltip.url}
                alt="Preview"
                className="max-w-[280px] max-h-[280px] rounded-lg object-contain"
                onError={(e: any) => (e.target.src = "https://via.placeholder.com/200?text=Error")}
            />
            </div>
        )}
