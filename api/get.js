...
import { proxyFetch } from "./_lib/proxyFetcher";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const proxiedOgImage = selectedImage ? `${protocol}://${host}/api/get?url=${encodeURIComponent(selectedImage)}` : "";
    const proxiedFavicon = favicon ? `${protocol}://${host}/api/get?url=${encodeURIComponent(favicon)}` : "";
    ...
    $("img").each((i, el) => {
      ...
      if (src) {
        ...
        if (resolved && !extractedImages.includes(resolved)) {
          extractedImages.push(`${protocol}://${host}/api/get?url=${encodeURIComponent(resolved)}`);
        }
      }
    });
